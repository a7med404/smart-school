      <!-- Bootstrap -->
      <link href="<?php echo e(asset('modules/master/plugins/bootstrap/css/bootstrap.min.css')); ?>" rel="stylesheet">
      
      <link href="<?php echo e(asset('modules/master/css/fontawesome.min.css')); ?>" rel="stylesheet">
      <link href="<?php echo e(asset('modules/master/css/line-scale.css')); ?>" rel="stylesheet">
      
      <link href="<?php echo e(asset('modules/master/css/toastr.min.css')); ?>" rel="stylesheet">
      
      <link href="<?php echo e(asset('modules/master/css/font-awesome.min.css')); ?>" rel="stylesheet">
      
      <!-- AdminLTE Skins. Choose a skin from the css/skins
      folder instead of downloading all of them to reduce the load. -->
      <link rel="stylesheet" href="<?php echo e(asset('modules/master/css/skins/_all-skins.min.css')); ?>">
      <!-- iCheck -->
      <link rel="stylesheet" href="<?php echo e(asset('modules/master/plugins/iCheck/all.css')); ?>">
      <!-- Morris chart -->
      <link rel="stylesheet" href="<?php echo e(asset('modules/master/plugins/morris/morris.css')); ?>">
      <!-- jvectormap -->
      <link rel="stylesheet" href="<?php echo e(asset('modules/master/plugins/jvectormap/jquery-jvectormap-1.2.2.css')); ?>">
      <!-- Date Picker -->
      <link rel="stylesheet" href="<?php echo e(asset('modules/master/plugins/datepicker/datepicker3.css')); ?>">
      <!-- Daterange picker -->
      <link rel="stylesheet" href="<?php echo e(asset('modules/master/plugins/daterangepicker/daterangepicker.css')); ?>">
      <!-- bootstrap wysihtml5 - text editor -->
      <link rel="stylesheet" href="<?php echo e(asset('modules/master/plugins/bootstrap-wysihtml5/bootstrap3-wysihtml5.min.css')); ?>">
      

      <!-- Timepicker -->
      <link href="<?php echo e(asset('modules/master/plugins/timepicker/bootstrap-timepicker.min.css')); ?>" rel="stylesheet">

      <!-- DataTables -->
      
      
<?php echo Html::style(asset('modules/master/plugins/datatables/jquery.dataTables.min.css')); ?> 
<?php echo Html::style(asset('modules/master/plugins/datatables/dataTables.bootstrap.min.css')); ?>


      <!-- Ionicons -->
      <link href="<?php echo e(asset('modules/master/plugins/ionicons/css/ionicons.min.css')); ?>" rel="stylesheet">
      <!-- Pace style -->
      <link rel="stylesheet" href="<?php echo e(asset('modules/master/plugins/pace/pace.min.css')); ?>">
      <!-- Bootstrap Color Picker -->
      <link rel="stylesheet" href="<?php echo e(asset('modules/master/plugins/colorpicker/bootstrap-colorpicker.min.css')); ?>">
      <!-- Bootstrap time Picker -->
      <link rel="stylesheet" href="<?php echo e(asset('modules/master/plugins/timepicker/bootstrap-timepicker.min.css')); ?>">
      <!-- Select2 -->
      <link rel="stylesheet" href="<?php echo e(asset('modules/master/plugins/select2/select2.min.css')); ?>">
      <!-- HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries -->
      <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
      <!--[if lt IE 9]>
      <script src="https://oss.maxcdn.com/html5shiv/3.7.3/html5shiv.min.js"></script>
      <script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
      <![endif]-->

      <link href="<?php echo e(asset('modules/master/css/AdminLTE.min.css')); ?>" rel="stylesheet">
      <link href="<?php echo e(asset('modules/master/css/AdminLTE-RTL.css')); ?>" rel="stylesheet">
      <link href="<?php echo e(asset('modules/master/css/framwork.css')); ?>" rel="stylesheet">
      <!-- CSRF Token -->
      <meta name="_token" content="<?php echo e(csrf_token()); ?>"/>
      <meta name="csrf-token" content="<?php echo e(csrf_token()); ?>">

      <!-- Main Font -->
      <script src="<?php echo e(asset('modules/master/js/webfontloader.min.js')); ?>"></script>
      <script>
      	WebFont.load({
      		google: {
      			families: ['Roboto:300,400,500,700:latin']
      		}
      	});
      </script>
<?php /**PATH /home/a7med404/a7meD404/WD_WORK/WorkingFolder/work-on/school-system/resources/views/cpanel/partials/styles.blade.php ENDPATH**/ ?>